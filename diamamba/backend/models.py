...
class BidirectionalMamba(torch.nn.Module):
    """ Bidirectional Mamba block to replace MultiHeadSelfAttention.

    Runs two Mamba SSMs (forward and backward) and combines via summation.
    Accepts (BT, D) input with batch_size argument, matching the
    MultiHeadSelfAttention interface.
    """
    ...
    def __call__(self, x: torch.Tensor, batch_size: int) -> torch.Tensor:
        x_3d = x.reshape(batch_size, -1, self.d_model)
        y_fwd = self.mamba_fwd(x_3d)
        y_bwd = self.mamba_bwd(x_3d.flip(dims=[1])).flip(dims=[1])
        y = y_fwd + y_bwd
        return y.reshape(-1, self.d_model)
    ...
    def __init__(
        ...
        if self.use_frame_selfattention:
            ...
            for i in range(self.n_layers):
                ...
                # Layer 3 (index 2) uses self-attention; others use BiMamba
                # attention_layer_indices = {3}
                attention_layer_indices = set(getattr(args, 'attention_layer_indices', [3]))
