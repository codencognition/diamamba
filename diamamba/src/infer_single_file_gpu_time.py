...
def parse_arguments() -> SimpleNamespace:
    ...
    parser.add_argument('--attention-layer-indices', type=list, default=[3])
...
if __name__ == '__main__':
    ...
    print(f"[torch.profiler] {prof_flops/1e9:.2f} GFLOPs (known ops only)")
    # print(prof.key_averages().table(sort_by="flops", row_limit=15))  # per-op detail

    if args.gpu >= 1:
        torch.cuda.reset_peak_memory_stats()  # keep your timing/memory block clean
    ...
    with torch.no_grad():
        ...
        end_event.record()
    torch.cuda.synchronize()
    gpu_time_s = start_event.elapsed_time(end_event) / 1000
    ...
    if args.plot_output:
        ...
        plt.cla()
        plt.clf()
    # print(f"[peak GPU] allocated {torch.cuda.max_memory_allocated()/1e9:.2f} GB | "f"reserved {torch.cuda.max_memory_reserved()/1e9:.2f} GB")
    audio_duration_s = librosa.get_duration(filename=filepath)
    print(f"[peak GPU] allocated {torch.cuda.max_memory_allocated()/1e9:.2f} GB | "
        f"reserved {torch.cuda.max_memory_reserved()/1e9:.2f} GB")
    print(f"[GPU time] {gpu_time_s:.3f} s | "
        f"RTF {gpu_time_s/audio_duration_s:.4f} "
        f"(~{audio_duration_s/gpu_time_s:.0f}x faster than real-time)")
