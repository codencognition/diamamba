    ...
    for epoch in range(int(round(init_epoch)), args.max_epochs):
        ...
        for i, batch in enumerate(train_loader):
            ...
            if i % args.log_report_batches_num == \
                ...
                print(f"Epoch {epoch+1}/{args.max_epochs} | Batch {i+1} | Loss: {loss.item():.4f} | LR: {get_rate(optimizer):.6f}")
            optimizer.zero_grad()
            loss.backward()
        ...
        print(f"=== Epoch {epoch+1}/{args.max_epochs} completed. Checkpoint saved. ===")
        ...
        n = dev_batches_qty
        print(f"=== Dev - Epoch {epoch+1} | Loss: {acum_dev_metrics['loss']/n:.4f} | DER: {acum_dev_metrics['DER']/n:.2f}% (miss: {acum_dev_metrics['DER_miss']/n:.2f}%, FA: {acum_dev_metrics['DER_FA']/n:.2f}%, conf: {acum_dev_metrics['DER_conf']/n:.2f}%) ===")
